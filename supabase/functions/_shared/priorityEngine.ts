// PriorityMail Edge Functions — Shared Priority Engine
// Identical deterministic logic to packages/priority-engine/src/index.ts
...
export function classifyEmail(
  ...
  for (const rule of activeRules) {
    ...
    switch (rule.type) {
      ...
      case 'label': {
        const labelMatch =
          (email.labelIds ?? []).some((l) => l.toLowerCase() === val) ||
